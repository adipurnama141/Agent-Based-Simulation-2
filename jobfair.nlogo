...
to setup-room
  set entrypoint? (distancexy 0 max-pycor) < 5
  ...
  [set jobbooth 1]
  setup-distance-to-jobfair
end

to setup-distance-to-jobfair
  set ns1 500 - 50 *( distancexy (0.6 * max-pxcor) 0)
  if (ns1 < 0) [set ns1 0]
  set ns2 500 - 50 * ( distancexy (-0.6 * max-pxcor) (-0.6 * max-pycor))
  if (ns2 < 0) [set ns2 0]
  set ns3 500 -  50 *(distancexy (-0.8 * max-pxcor) (0.8 * max-pycor))
  if (ns3 < 0) [set ns3 0]
  set distance-to-jobbooth ns1 + ns2 + ns3
end
...
to go  ;; forever button
  ask turtles
  [ if who >= ticks [ stop ] ;; masuk berurutan sesuai antrian
    ifelse (tingkat = 1)
    [ ifelse color = red
    [ look-for-jobbooth]
    [ look-for-another-jobbooth]
    wiggle
    fd 1 ]
    [
    set distance-to-friend distance-to-friend + 60
    ifelse color = red
    [look-for-jobbooth]
    [return-to-friend]
    wiggle
  ...
  diffuse distance-to-friend (diffusion-rate / 100)
  ask patches
  [ set distance-to-friend distance-to-friend * (100 - excitement) / 100  ;; slowly evaporate chemical
    setup-distance-to-jobfair
    ifelse (excitement > 0)
    [ set excitement (excitement - 0.00001)]
    [ set excitement 0]
    ]
  tick
end

to look-for-jobbooth  ;; turtle procedure
  uphill-distance-to-jobbooth
  if jobbooth > 0
  [ set color green     ;; pick up food
    stop ]
end

to look-for-another-jobbooth
  uphill-distance-to-jobbooth
end

to return-to-friend
  uphill-distance-to-friend
end


;; sniff left and right, and go where the strongest smell is
to uphill-distance-to-friend  ;; turtle procedure
  let scent-ahead distance-to-friend-scent-at-angle   0
  let scent-right distance-to-friend-scent-at-angle  45
  let scent-left  distance-to-friend-scent-at-angle -45
...
;; sniff left and right, and go where the strongest smell is
to uphill-distance-to-jobbooth  ;; turtle procedure
  let scent-ahead distance-to-jobbooth-at-angle   0
  let scent-right distance-to-jobbooth-at-angle  45
  let scent-left  distance-to-jobbooth-at-angle -45
  if (scent-right > scent-ahead) or (scent-left > scent-ahead)
  [ ifelse scent-right > scent-left
    [ rt 45 ]
    [ lt 45 ] ]
...
to-report distance-to-jobbooth-at-angle [angle]
  let p patch-right-and-ahead angle 1
  if p = nobody [ report 0 ]
  report [distance-to-jobbooth] of p
end

to-report distance-to-friend-scent-at-angle [angle]
  let p patch-right-and-ahead angle 1
  if p = nobody [ report 0 ]
  report [distance-to-friend] of p
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot 311 - count turtles with [color = green]"
